...
export default function AddProjectModal({ onClose, onSave, projectToEdit, onAssignMembers }) {
  ...
  setCreatedProjectId(projectToEdit.project_id); // ✅
  projectId = projectToEdit.project_id;
} else {
  const res = await addProject(payload);
  setCreatedProjectId(res.message); // ✅
  projectId = res.message;
  localStorage.removeItem("project-draft");
}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30 backdrop-blur-sm">
            <div className="bg-[#F0E4D3] p-6 rounded-lg shadow-lg w-[90%] max-w-2xl max-h-screen overflow-y-auto relative">
                ...
                <div className="mb-4 flex justify-end">
                    <button
                        onClick={() =>
                            onAssignMembers(projectToEdit?.project_id || form?.temp_id || null)
                        }
